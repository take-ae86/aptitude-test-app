...
import '../../logic/three_sentence_completion_engine.dart';

class SpiThreeSentenceCompletionPage extends StatefulWidget {
  const SpiThreeSentenceCompletionPage({super.key});

  @override
  State<SpiThreeSentenceCompletionPage> createState() => _SpiThreeSentenceCompletionPageState();
}

class _SpiThreeSentenceCompletionPageState extends State<SpiThreeSentenceCompletionPage> {
  final ThreeSentenceCompletionEngine _engine = ThreeSentenceCompletionEngine();
  List<ThreeSentenceProblem> _problems = [];
  ...
  Future<void> _startQuiz() async {
    ...
    try {
      ...
      for (var p in problems) {
        ...
        // 正解
        options.add(OptionSet(answers: p.correctOrder, isCorrect: true, label: ""));
        // 誤答
        List<List<String>> wrongs = _engine.generateWrongOptions(p.correctOrder, 4);
        for (var w in wrongs) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: const Text('空欄補充（三文完成）', style: TextStyle(fontSize: 16)),
          actions: [
            if (!_isLoading)
              Container(
      ...
      children: [
        ...
        Expanded(
          child: SingleChildScrollView(
            padding: const EdgeInsets.all(12), // 余白削減
            child: Column(
              ...
              children: [
                Container(
                  ...
                  child: Text('第${_currentIndex + 1}問', style: TextStyle(color: Colors.orange[900], fontWeight: FontWeight.bold, fontSize: 13)),
                ),
                ...
                Card(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: problem.sentences.map((s) => Padding(
                        padding: const EdgeInsets.only(bottom: 6),
                        child: Text(
                          s, 
                          style: const TextStyle(fontSize: 14, color: Colors.black87, height: 1.4, fontWeight: FontWeight.w500) // 14, 1.4
                        ),
                      )).toList(),
                    ),
                  ),
                ...
                // 選択肢A-E
                Container(
                  padding: const EdgeInsets.symmetric(vertical: 6, horizontal: 8), // 6, 8
                    ...
                    children: problem.options.entries.map((entry) {
                      return Padding(
                        ...
                        child: Row(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(entry.key, style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.orange, fontSize: 13)), // 13
                            const SizedBox(width: 8),
                            Expanded(child: Text(entry.value, style: const TextStyle(fontSize: 13, height: 1.3))), // 13
                          ],
                        ),
                      );
                    }).toList(),
                  ...
                  child: SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      style: ElevatedButton.styleFrom(
                        ...
                        minimumSize: const Size(double.infinity, 40), // 40
                      ),
                      child: Row(
                        children: [
                          ...
                          Text(
                            ...
                            style: const TextStyle(fontSize: 13, color: Colors.black87, fontWeight: FontWeight.w500), // 13
                          ),
                        ],
                      ),
                    ),
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final uAnsLabel = _userAnswers[index];
              final options = _currentOptionsList[index];
              
              OptionSet? selected = options.firstWhere((o) => o.label == uAnsLabel, orElse: () => OptionSet(answers: [], isCorrect: false, label: "-"));
              final isCorrect = selected.isCorrect;
              OptionSet correctOption = options.firstWhere((o) => o.isCorrect);
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // 問題文を少し表示
                          Text(problem.sentences[0].substring(0, min(20, problem.sentences[0].length)) + "...", style: const TextStyle(fontSize: 12)),
                          const SizedBox(height: 8),
                          
                          Row(
                            children: [
                              const Text('あなたの回答: ', style: TextStyle(fontSize: 12, color: Colors.grey)),
                              Text("${selected.label} ${_formatOrderString(selected.answers)}", style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: isCorrect ? Colors.green : Colors.red)),
                            ],
                          ),
                          ...
                          Row(
                            children: [
                              ...
                              Text("${correctOption.label} ${_formatOrderString(correctOption.answers)}", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.green)),
                            ],
                          ),
