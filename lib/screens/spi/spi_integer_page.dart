...
import '../../logic/spi_integer/integer_engine.dart'; // Dedicated engine

class SpiIntegerPage extends StatefulWidget {
  const SpiIntegerPage({super.key});

  @override
  State<SpiIntegerPage> createState() => _SpiIntegerPageState();
}

class _SpiIntegerPageState extends State<SpiIntegerPage> {
  final IntegerEngine _engine = IntegerEngine();
  ...
  void _handleOptionSelected(String selectedOption) {
    ...
    setState(() {
      // If it's "A ア...", store 'A'. Else store full text.
      if (selectedOption.startsWith("A ") || selectedOption.startsWith("B ") || selectedOption.startsWith("C ")) {
         _userAnswers[_currentIndex] = selectedOption.substring(0, 1);
      } else {
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: const Text('整数の推測', style: TextStyle(fontSize: 16)),
          actions: [
            if (!_isLoading)
              Container(
  ...
  Widget _buildQuestionView() {
    ...
    // Check if options are Condition Type (A..E)
    bool isConditionType = problem.options[0].startsWith("A ");
                    ...
                    return Padding(
                      ...
                      child: SizedBox(
                        ...
                        child: ElevatedButton(
                          ...
                          child: isConditionType ? 
                            // Condition Type: "A アだけで..."
                            Row(
                              children: [
                                Text(option.substring(0, 1), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.blue)),
                                const SizedBox(width: 12),
                                Expanded(
                                  child: Text(option.substring(2), 
