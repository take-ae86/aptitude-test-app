...
import '../../logic/condition_engine.dart'; // Point to dedicated Condition engine

class SpiInferenceConditionPage extends StatefulWidget {
  const SpiInferenceConditionPage({super.key});

  @override
  State<SpiInferenceConditionPage> createState() => _SpiInferenceConditionPageState();
}

class _SpiInferenceConditionPageState extends State<SpiInferenceConditionPage> {
  final ConditionEngine _engine = ConditionEngine(); // Use dedicated engine
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: const Text('推論（答えが決まる条件）', style: TextStyle(fontSize: 16)),
          actions: [
            if (!_isLoading)
              Container(
                    ...
                    children: [
                      ...
                      Column(
                        children: problem.options.map((option) {
                          ...
                          String content = option.substring(2);
                          return Padding(
  ...
  Widget _buildResultView() {
    ...
    return Column(
      children: [
        // Same Compact Header as Numerical
        Padding(
