...
import '../../logic/numerical_engine.dart'; // Point to dedicated Numerical engine

class SpiInferenceNumericalPage extends StatefulWidget {
  const SpiInferenceNumericalPage({super.key});

  @override
  State<SpiInferenceNumericalPage> createState() => _SpiInferenceNumericalPageState();
}

class _SpiInferenceNumericalPageState extends State<SpiInferenceNumericalPage> {
  final NumericalEngine _engine = NumericalEngine(); // Use dedicated engine
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: const Text('推論・数値算出', style: TextStyle(fontSize: 16)),
          actions: [
            if (!_isLoading)
              Container(
  ...
  Widget _buildResultView() {
    ...
    return Column(
      children: [
        ...
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4), // Minimal outer padding
          child: Column(
            children: [
              Container(
                padding: const EdgeInsets.symmetric(vertical: 8, horizontal: 16), // Aggressively reduced inner padding
        ...
        Expanded(
          child: ListView.builder(
            padding: const EdgeInsets.fromLTRB(16, 0, 16, 10), // Zero top padding
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                margin: const EdgeInsets.only(bottom: 4), // Tight margin
                elevation: 1,
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
                child: ExpansionTile(
                  dense: true, 
