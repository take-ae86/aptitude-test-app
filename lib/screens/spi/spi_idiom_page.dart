...
import '../../logic/idiom_engine.dart';

class SpiIdiomPage extends StatefulWidget {
  const SpiIdiomPage({super.key});

  @override
  State<SpiIdiomPage> createState() => _SpiIdiomPageState();
}

class _SpiIdiomPageState extends State<SpiIdiomPage> {
  final IdiomEngine _engine = IdiomEngine();
  List<IdiomProblem> _problems = [];
  ...
  List<List<OptionSet>> _currentOptionsList = [];
  ...
  Future<void> _startQuiz() async {
    ...
    try {
      ...
      List<List<OptionSet>> optionsList = [];
      for (var p in problems) {
        List<OptionSet> options = [];
        options.add(OptionSet(answers: p.correctAnswers, isCorrect: true, label: ""));
        List<List<String>> wrongs = _engine.generateWrongOptions(p.correctAnswers, 4);
        for (var w in wrongs) {
      ...
      if (mounted) {
        setState(() {
          _problems = problems;
          _currentOptionsList = optionsList;
  ...
  void _handleOptionSelected(String selectedLabel) {
    if (_isFinished) return;
    setState(() {
      _userAnswers[_currentIndex] = selectedLabel;
    });
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: const Text('熟語の成り立ち', style: TextStyle(fontSize: 16)),
          actions: [
            if (!_isLoading)
              Container(
  ...
  Widget _buildQuestionView() {
    ...
    final options = _currentOptionsList[_currentIndex];
            ...
            child: Column(
              ...
              children: [
                ...
                Card(
                  ...
                  child: Padding(
                    padding: const EdgeInsets.all(10.0), // 16 -> 10
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text("次の(1)〜(5)の熟語の成り立ちとして当てはまるものを、AからDの中から選びなさい。", style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold)),
                        const SizedBox(height: 8), // 12 -> 8
                        Wrap(
                          spacing: 12, // 16 -> 12
                          runSpacing: 4, // 8 -> 4
                          children: List.generate(problem.idioms.length, (i) {
                            return Text("(${i+1}) ${problem.idioms[i]}", style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold)); // 16 -> 15
                          }),
                        ),
                      ],
                    ),
                ...
                // 選択肢の定義
                Container(
                  padding: const EdgeInsets.symmetric(vertical: 6, horizontal: 8), // 12 -> 6, 8
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      _buildDefinitionRow("A", problem.optionDefinitions["A"]!),
                      _buildDefinitionRow("B", problem.optionDefinitions["B"]!),
                      _buildDefinitionRow("C", problem.optionDefinitions["C"]!),
                      _buildDefinitionRow("D", problem.optionDefinitions["D"]!),
                    ],
                  ),
                ),
                ...
                ...options.map((opt) => Padding(
                  padding: const EdgeInsets.only(bottom: 2), // 12 -> 2
                  child: SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      style: ElevatedButton.styleFrom(
                        ...
                        padding: const EdgeInsets.symmetric(vertical: 8, horizontal: 12), // 12 -> 8
                      ...
                      child: RichText(
                        text: TextSpan(
                          children: [
                            TextSpan(text: "${opt.label}  ", style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.orange)), // 16 -> 14
                            TextSpan(
                              text: _formatAnswerString(opt.answers),
                              style: const TextStyle(fontSize: 13, color: Colors.black87, fontWeight: FontWeight.w500), // 14 -> 13
                            ),
                          ],
                        ),
                      ),
  ...
  String _formatAnswerString(List<String> answers) {
    StringBuffer sb = StringBuffer();
    for (int i = 0; i < answers.length; i++) {
      sb.write("(${i+1})${answers[i]} ");
    }
  ...
  Widget _buildDefinitionRow(String label, String text) {
    return Padding(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(label, style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.orange, fontSize: 13)),
  ...
  Widget _buildResultView() {
    ...
    for (int i = 0; i < _problems.length; i++) {
      OptionSet? selected = _currentOptionsList[i].firstWhere((o) => o.label == _userAnswers[i], orElse: () => OptionSet(answers: [], isCorrect: false, label: ""));
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              OptionSet? selected = _currentOptionsList[index].firstWhere((o) => o.label == _userAnswers[index], orElse: () => OptionSet(answers: [], isCorrect: false, label: "-"));
              final isCorrect = selected.isCorrect;
              OptionSet correctOption = _currentOptionsList[index].firstWhere((o) => o.isCorrect);
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text("熟語: ${problem.idioms.join(", ")}", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                          const SizedBox(height: 8),
                          
                          Row(
                            children: [
                              const Text('あなたの回答: ', style: TextStyle(fontSize: 12, color: Colors.grey)),
                              Text("${selected.label} ${_formatAnswerString(selected.answers)}", style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: isCorrect ? Colors.green : Colors.red)),
                            ],
                          ),
                          ...
                          Row(
                            children: [
                              ...
                              Text("${correctOption.label} ${_formatAnswerString(correctOption.answers)}", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.green)),
                            ],
                          ),
                          ...
                          ...List.generate(5, (i) {
                            return Padding(
                              ...
                              child: Text("(${i+1}) ${problem.idioms[i]}: ${problem.explanations[i]}", style: const TextStyle(fontSize: 12)),
                            );
                          }),
                        ],
                      ),
